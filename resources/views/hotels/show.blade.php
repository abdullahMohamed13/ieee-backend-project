    ...
    <div class="py-8 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @php
                $selectedImage = 0;
                $amenityIcons = [
                    'WiFi' => 'wifi', 'Restaurant' => 'utensils', 'Gym' => 'dumbbell',
                    'Pool' => 'waves', 'Parking' => 'car', 'Spa' => 'sparkles',
                ];
            @endphp
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-2 p-2">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-1 gap-2">
                        ...
                        <img src="{{ $image }}" alt="{{ $hotel['name'] }} {{ $index + 1 }}"
                             onclick="document.getElementById('main-image').src = this.src; document.querySelectorAll('.thumb').forEach(t => t.classList.remove('ring-4')); this.classList.add('ring-4');"
                             class="thumb w-full h-[120px] lg:h-[120px] object-cover rounded-lg cursor-pointer hover:opacity-75 transition-opacity {{ $index === 0 ? 'ring-4 ring-blue-900' : '' }}">
                        @endforeach
                    </div>
                </div>
            </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                            ...
                            <div class="flex items-center space-x-3">
                                <div class="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center text-blue-900">
                                    <i data-lucide="{{ $amenityIcons[$amenity] ?? 'star' }}" class="w-5 h-5"></i>
                                </div>
                                    ...
                                    <tr class="border-b hover:bg-gray-50">
                                        <td class="py-4 px-4">
                                            <div class="flex items-center space-x-2">
                                                <i data-lucide="bed" class="w-5 h-5 text-gray-600"></i>
                                        ...
                                        <td class="py-4 px-4">
                                            @if($room['available'])
<x-button href="{{ url('/booking/' . $hotel['id']) }}" size="sm">Book Now</x-button>
                                            @endif
                                        </td>
                                    </tr>
                                    @endforeach
...
<x-button href="{{ url('/booking/' . $hotel['id']) }}" block class="mb-4">Book Now</x-button>

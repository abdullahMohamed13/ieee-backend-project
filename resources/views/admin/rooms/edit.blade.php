<x-layout>
<x-slot:title>Edit Room - {{ $room->hotel->name }}</x-slot:title>
        ...
        <div class="bg-white rounded-xl shadow-lg p-8">
            ...
            <form action="{{ route('admin.rooms.update', [$hotel, $room]) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="room_number" id="room_number" value="{{ old('room_number', $room->room_number) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-900 @error('room_number') border-red-500 @enderror">
                    @error('room_number')
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="price" id="price" value="{{ old('price', $room->price) }}" step="0.01" min="0" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-900 @error('price') border-red-500 @enderror">
                    @error('price')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="capacity" id="capacity" value="{{ old('capacity', $room->capacity) }}" min="1" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-900 @error('capacity') border-red-500 @enderror">
                    @error('capacity')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-end space-x-4">
                    ...
                    <button type="submit" class="px-6 py-2 bg-blue-900 text-white rounded-lg hover:bg-blue-800 font-semibold transition-colors">Update Room</button>
                </div>
            </form>
        </div>
    </div>
